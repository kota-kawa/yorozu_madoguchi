/**
 * EN: Provide the useJobChat module implementation.
 * JP: useJobChat モジュールの実装を定義する。
 */
...
/**
 * EN: Declare the useJobChat value.
 * JP: useJobChat の値を宣言する。
 */
export const useJobChat = () => {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
       ...
       * JP: response の値を宣言する。
       */
      const response = await fetch(apiUrl('/job_send_message'), {
        method: 'POST',
